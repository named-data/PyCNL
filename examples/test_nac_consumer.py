...
def createFace(contentName, uKeyName, fixtureUDKeyBlob, keyChain, certificateName):
    """
    Create a Face and the packets to fetch from it.
    """
    ...
    encryptParams = EncryptParams(EncryptAlgorithmType.RsaOaep)
    Encryptor.encryptData(
      dKeyData, fixtureDKeyBlob, uKeyName, fixtureUEKeyBlob,
      encryptParams)
    ...
    return TestFace(handleExpressInterest)
...
def main():
    ...
    # Generate the user key.
    fixtureUDKeyBlob = RsaAlgorithm.generateKey(RsaKeyParams()).getKeyBits()


    # Imitate test_consumer from the PyNDN integration tests.
    (keyChain, certificateName) = createKeyChain()
    face = createFace(
      contentName, uKeyName, fixtureUDKeyBlob, keyChain, certificateName)
    ...
    try:
        ...
        pass
    consumer = Consumer(
      face, keyChain, groupName, uName, Sqlite3ConsumerDb(databaseFilePath))
    consumer.addDecryptionKey(uKeyName, fixtureUDKeyBlob)

    def onConsumeComplete(data, result):
        dump("Debug result", result.equals(Blob(DATA_CONTENT, False)))
        
    consumer.consume(
      contentName, onConsumeComplete,
      lambda code, message:
