"""Test helpers related to Face."""

import time
from pyndn import Name
from pyndn.security import KeyChain, SafeBag
from pyndn.util import Blob

DEFAULT_RSA_PUBLIC_KEY_DER = bytearray([
    
])

# Use an unencrypted PKCS #8 PrivateKeyInfo.
DEFAULT_RSA_PRIVATE_KEY_DER = bytearray([
    
])


def loadDefaultKey(keyChain):
    """Load a default RSA key for testing purpose."""
    keyChain.importSafeBag(SafeBag
                           (Name("/testname/KEY/123"),
                            Blob(DEFAULT_RSA_PRIVATE_KEY_DER, False),
                               Blob(DEFAULT_RSA_PUBLIC_KEY_DER, False)))


defaultKeyChain = KeyChain("pib-memory:", "tpm-memory:")
loadDefaultKey(defaultKeyChain)


def getDefaultKeyChain():
    """Retrieve a KeyChain with the default key loaded."""
    return defaultKeyChain


def setCommandSigningInfo(face):
    """Set CommandSigningInfo on a Face."""
    face.setCommandSigningInfo(
        defaultKeyChain, defaultKeyChain.getDefaultCertificateName())


def errorOnRegisterFailed(prefixName):
    """"Raise an assertion error upon prefix registration failure."""
    assert False, "Prefix registration failed"


wantStopProcessEvents = [False]


def processEvents(faces, interval=0.01, duration=None):
    """
    Call processEvents on one or more faces.

    :param list faces: a list of Face objects.
    :param float interval: sleep duration between processEvents invocations.
    :param float|None duration: total duration, None for infinite.
    """
    endTime = float("inf") if duration is None else time.time() + duration
    wantStopProcessEvents[0] = False
    while not wantStopProcessEvents[0] and time.time() < endTime:
        for face in faces:
            face.processEvents()
        time.sleep(0.01)


def stopProcessEvents():
    """Stop processEvents() loop of this module."""
    wantStopProcessEvents[0] = True
